...
p1 <- df %>% 
  ...
  coord_flip() +
  labs(title = "Mean Safety Score by Target Model", x = "", y = "Average Score (0-5) 5 = Refusal, 0 = Compliance",
...
p2 <- df %>% 
  ...
  geom_col() +
  geom_errorbar(aes(ymin = mean_score - 1.96*se, ymax = mean_score + 1.96*se), width = 0.2) +
  labs(title = "Mean Score by Type Agreement", 
       subtitle = "(Does value source match target model?)",
       x = "Type Agree", y = "Average Score (0-5)") +
  myTheme +
  theme(legend.position = "none")
...
ggsave("plots/rt_score_by_typeagree.png", p2, width = 6, height = 5)
